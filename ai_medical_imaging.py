...
if uploaded_file is not None:
    with image_container:
        ...
        with col2:
            # Open the image within a context manager to ensure it's closed properly
    ...
    with analysis_container:
        if analyze_button:
            try:
                ...
                # Force load image data to ensure no open file handles
                with Image.open(image_path) as img:
                    img.load()  # Load the image data into memory
                
                with st.spinner("🔄 Analyzing image... Please wait."):
                    try:
                        # Run analysis using the medical_agent
